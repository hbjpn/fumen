...
render_impl(canvas, track, param)
{
	...
	render_measure_row_simplified(x, paper, macros,
		...
		first_block_first_row, inner_reharsal_mark, music_context)
{
	...
	var y_next_base = y_ml_area_base
		+ (ml_area_detected ? Math.max(1,lyric_rows) * param.ml_row_height : 0 )
		...
		if(elements.body.length==0){
			...
			elements.body.forEach(function(e){
				if(e instanceof Chord){
					var cr = render_chord_simplified(false, e, transpose, half_type, paper, x, y_body_base,
							param, C7_width);
	...
	for(var ml = 0; ml < row_elements_list.length; ++ml){
		...
		if(elements.body.length == 0){
			...
			elements.body.forEach(function(e){
				if(e instanceof Chord){
					var cr = render_chord_simplified(true, e, transpose, half_type, paper, x, y_body_base,
							param, C7_width);
...
CanvasText(canvas, x, y, text, fsize, align, xwidth, notdraw, opt)
{
	...
	if(false){
		...
		CanvasLine(canvas, x, y + yadjust, x + xwidth, y + yadjust, {dash:true});
...
render_chord_as_string_plain(chord, paper, x, y_body_base,
	param, draw)
{
...
render_rest_plain(e, paper, draw, x, y_body_or_rs_base, C7_width, _5i, param)
{
	var yoffsets = {1:-_5i, 2:-_5i/6*4, 4:-_5i*1.5, 8:-_5i, 16:0, 32:0, 64:_5i};
	var heights =   {1:_5i/6*4, 2:_5i/6*4, 4:_5i*3, 8:_5i*2.5, 16:_5i*2.5, 32:_5i*2.5, 64:_5i*2.5};
...
render_simile_mark_plain(draw, paper, x, y_body_base, row_height, numslash, put_boundary, align)
{
	var h = 4;
	var H = 12;
	var i = 4;
...
render_chord_simplified(draw, chord, transpose, half_type, canvas, x, y_body_base,
	param, C7_width)
{
	...
	var width =  Math.max(upper_width, lower_width, onbass_width) + tensions_width;
 ...
 */
draw_boundary_simplified(side, e0, e1, hasNewLine, canvas, x, y_body_base, param, draw)
{
	...
	case 'b':
		// begin only
		w = 8;
		if(draw) CanvasLine(canvas, x, y_body_base, x, y_body_base + row_height, {width:2});
	...
	case 'B':
		...
		if(draw) CanvasLine(canvas, x+7, y_body_base, x+7, y_body_base + row_height, {width:2});
...
JudgeTextYPosOffset(canvas, bold, fontfamily, fontsize)
{
	...
	return {top_room:top_room/G_pixelRatio,height:M_height/G_pixelRatio};
}
