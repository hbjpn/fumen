...
function Sequencer(track, cb_play, cb_stop, param) {
    ...
    var next = function(c) {
        do {
            if (c.rgi >= track.reharsal_groups.length) return null;
            c.mi++;
            ...
            if (e instanceof LoopIndicator) {
                if (cur_loop) {
                    if (e.intindicators.indexOf(cur_loop.cnt) >= 0) {
                        ...
                        while ((m = next(ctx))) {
                            if (
                                findElement(m, function(ec) {
                                    return (
                                        ec instanceof LoopIndicator &&
                                        ec.intindicators.indexOf(
                                            cur_loop.cnt
                                        ) >= 0
                                    );
                                })
            ...
            if (e instanceof LoopEndMark || e instanceof LoopBothMark) {
                ...
                if (e.number in segnos) {
                    ...
                    for (var key in tocodas) {
                        if (
                            ctxlt(tocodas[key].p, ctx) &&
                            ctxlte(segnos[e.number].p, tocodas[key].p)
                        )
                ...
                if (e.number in tocodas && tocodas[e.number].valid) {
                    ...
                    while ((m = next(ctx))) {
                        if (
                            findElement(m, function(ec) {
                                return (
                                    ec instanceof Coda && ec.number == e.number
                                );
                            })
                        ) {
                            break;
...
Sequencer.prototype.onClock = function() {
    ...
    for (; this.cmi < seq.length; ++this.cmi) {
        if (
            diff >= seq[this.cmi][0].t * measure_length &&
            diff <
                (seq[this.cmi][0].t + seq[this.cmi][0].duration) *
                    measure_length
        ) {
            break;
        }
    ...
    this.lastindicator = m.renderprop.paper
        .rect(sx, y, ex - sx, 30)
